            ...
            try
            {
                ...
                var vm = new AdminDashboardViewModel
                {
                    ...
                    PreinscripcionesPorCarrera = preinscripciones
                        .Where(p => !string.IsNullOrWhiteSpace(p.academiccontrol_preinscription_careerRequested))
                        .GroupBy(p => p.academiccontrol_preinscription_careerRequested)
                        .ToDictionary(g => g.Key, g => g.Count()),

                    AspirantesPorCarrera = aspirantes
                        .Where(a => !string.IsNullOrWhiteSpace(a.academiccontrol_inscription_careerRequested))
                        .GroupBy(a => a.academiccontrol_inscription_careerRequested)
                        .ToDictionary(g => g.Key, g => g.Count()),

                    PreinscripcionesPorEstado = preinscripciones
                        .Where(p => !string.IsNullOrWhiteSpace(p.Domicilio?.academiccontrol_preinscription_address_state))
                    ...
                    AspirantesPorEstado = aspirantes
                        .Where(a => !string.IsNullOrWhiteSpace(a.Preinscripcion?.Domicilio?.academiccontrol_preinscription_address_state))
