...
namespace ControlEscolar.Data
{
	public class Management
	{
		...
		private const string UserDashboardQuery = @"
			...
			WHERE (@ID IS NULL OR u.management_user_ID = @ID OR t.management_teacher_ID = @ID)
			  AND (@Status IS NULL OR ISNULL(u.management_user_status, 1) = @Status)
        ...
        private const string GetCareersQuery = @"
        ...
        public async Task<Dictionary<int, string>> GetCareersAsync()
        {
            ...
            return careers.GroupBy(c => c.Id).ToDictionary(g => g.Key, g => g.First().Name);
        }
        ...
        public async Task<List<BitacoraViewModel>> GetBitacoraAsync()
        {
            return await ExecuteQueryAsync(BitacoraQuery, null, reader => new BitacoraViewModel
            {
                ...
                Usuario = GetValue<string>(reader, "Usuario"),
                NombreCompleto = GetValue<string>(reader, "NombreCompleto"),
                Accion = GetValue<string>(reader, "Accion")
            });
        }
